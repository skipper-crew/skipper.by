		...
		if ( $qpw_user_can_publish_posts == 'yes' ) {
			...
			?>
				...
				<form autocomplete="off" method="post" name=quickpostwidget action="" >
					...
					<input type="hidden" id="quick_post_dbg_abspath" value="<?php echo ABSPATH; ?>" />
					<input type="hidden" id="quick_post_dbg_uploads" value="<?php echo UPLOADS; ?>" />
					<input type="hidden" id="quick_post_dbg_upath" value="<?php echo $_SESSION['upath']; ?>" />
					<input type="hidden" id="quick_post_dbg_qpw_upload_dir" value="<?php echo qpw_upload_dir(); ?>" />
					<input type="hidden" id="quick_post_dbg_qpw_document_root" value="<?php echo $_SERVER['DOCUMENT_ROOT']; ?>" />
			...
			if ($_POST) {
				...
				if ($error == 'no') {
					...
					$post_id = wp_insert_post( array(
						...
						'post_date_gmt'		=> ( $_POST['quick_post_date'] != '' ) ? get_gmt_from_date($_POST['quick_post_date']) : ''
					) );
